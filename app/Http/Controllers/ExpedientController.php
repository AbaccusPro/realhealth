...
class ExpedientController extends Controller
{
    public function expedient($Id){
        ...
        $user = User::find($id); //se busca el usuarioa  través del id
        //se hace una evaluacion para saber si el usuario ya ha comenzado a guardar ciertos datos de su expediente
        if($user->information != null){
            //si ya ha contestado algo se regresara la vista de edición para que pueda seguir completando
            return view('users.history.edit', compact('user'));
        }
        //si no se regresa la vista de creacion...
    ...
    public function storeExpedient(ExpedientCreateRequest $request, $id){
        ...
        $user = User::find($id);//se busca el usuario con el id        

        if ($user->information != null) { //se evalua si el usuario tiene algo guardado en su expediente
            //de ser asi se buscan los modelos que se van a editar, esto en base a los ids y a su relacion que tienen entre si
            ...
            //si no tiene nada guardado significa que el usuario no ha guardado nada y es la priemra vez que entra al formulario, en ese caso se crean las variables de los modelos que se van guardar
        ...
        /*posterioremnete se hace referencia al elemento del modelo, es decir los atributos que tiene por ejemplo
        $info->Name hace referencia al campo name de la tabla que tiene asignada el modelo Information... luego a ese atributo se le asigna el valor obtenido del formulario del campo con el mismo nombre:
        ...
        */
    	...
    	$info->User_id				= \Auth::user()->id; // se asigna la relacion con el id del usuario en sesión.
    	$info->save();
        ...
        \Session::flash('message', 'Expedient Create successfully');//mensaje de confirmación
    ...
    public function showExpedient($Id){
        //metodo para ver los expedientes...
        $id = base64_decode($Id); //se decodifica el id
        $user = User::find($id); //se busca el usuario
        //se manda a ala vista con el usuario
        return view('users.history.edit', compact('user'));
    }
}
